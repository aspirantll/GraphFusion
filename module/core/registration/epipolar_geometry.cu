...
namespace rtf {
    __global__ void computeRMSKernel(CUDAPtrs mat, CUDAPtrs meanPoint, CUDAPtrs summator) {
        ...
        Scalar point[3]={mat(index, 0), mat(index, 1), mat(index, 2)};
        Scalar item = (point[0]-meanPoint[0])*(point[0]-meanPoint[0]) + (point[1]-meanPoint[1])*(point[1]-meanPoint[1])
    ...
    __global__ void transformPointKernel(CUDAPtrs mat, CUDAPtrs transform) {
        ...
        Scalar x = transform(0,0)*point[0]+transform(0,1)*point[1]+transform(0,2)*point[2];
        Scalar y = transform(1,0)*point[0]+transform(1,1)*point[1]+transform(1,2)*point[2];
        Scalar z = transform(2,0)*point[0]+transform(2,1)*point[1]+transform(2,2)*point[2];
    ...
    __global__ void computeAMatrixKernel(CUDAPtrs x, CUDAPtrs y, CUDAPtrs A) {
        ...
        Scalar pointX[3]={x(index, 0), x(index, 1), x(index, 2)};
        Scalar pointY[3]={y(index, 0), y(index, 1), y(index, 2)};
    ...
    __global__ void computeResidualKernel(CUDAPtrs x, CUDAPtrs y, CUDAPtrs E, CUDAPtrs residuals) {
        ...
        Scalar xtEy = pointY[0]*(E(0,0)*pointX[0]+E(1,0)*pointX[1]+E(2,0)*pointX[2])
                    + pointY[1]*(E(0,1)*pointX[0]+E(1,1)*pointX[1]+E(2,1)*pointX[2])
                    + pointY[2]*(E(0,2)*pointX[0]+E(1,2)*pointX[1]+E(2,2)*pointX[2]);

        Scalar E1y = E(0,0)*pointY[0]+E(0,1)*pointY[1]+E(0,2)*pointY[2];
        Scalar E2y = E(1,0)*pointY[0]+E(1,1)*pointY[1]+E(1,2)*pointY[2];
        Scalar Et1x = E(0,0)*pointX[0]+E(1,0)*pointX[1]+E(2,0)*pointX[2];
        Scalar Et2x = E(0,1)*pointX[0]+E(1,1)*pointX[1]+E(2,1)*pointX[2];

        Scalar residual = xtEy*xtEy/(E1y*E1y + E2y*E2y + Et1x*Et1x + Et2x*Et2x);
//        printf("cuda: %ld %f\n", index, residual);
    ...
    __global__ void computeHomoAMatrixKernel(CUDAPtrs x, CUDAPtrs y, CUDAPtrs A) {
        ...
        const Scalar s_0 = pointY[0];
        const Scalar s_1 = pointY[1];
        const Scalar d_0 = pointX[0];
        const Scalar d_1 = pointX[1];
    ...
    __global__ void computeHomoResidualKernel(CUDAPtrs x, CUDAPtrs y, CUDAPtrs H, CUDAPtrs residuals) {
        ...
        const Scalar H_00 = H(0, 0);
        const Scalar H_01 = H(0, 1);
        const Scalar H_02 = H(0, 2);
        const Scalar H_10 = H(1, 0);
        const Scalar H_11 = H(1, 1);
        const Scalar H_12 = H(1, 2);
        const Scalar H_20 = H(2, 0);
        const Scalar H_21 = H(2, 1);
        const Scalar H_22 = H(2, 2);
        ...
        const Scalar pd_0 = H_00 * s_0 + H_01 * s_1 + H_02;
        const Scalar pd_1 = H_10 * s_0 + H_11 * s_1 + H_12;
        const Scalar pd_2 = H_20 * s_0 + H_21 * s_1 + H_22;

        const Scalar inv_pd_2 = 1.0 / pd_2;
        const Scalar dd_0 = d_0 - pd_0 * inv_pd_2;
        const Scalar dd_1 = d_1 - pd_1 * inv_pd_2;

        Scalar residual = dd_0 * dd_0 + dd_1 * dd_1;
//        printf("cuda: %ld %f\n", index, residual);
        residuals.set(index, 0, residual);
    }
