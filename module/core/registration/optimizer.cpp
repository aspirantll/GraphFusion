...
public:
    ...
    Averaging3dErrorTerm(const CeresPose& t_ab_measured, const float weight)
            : t_ab_measured_(t_ab_measured), weight(weight){}
                    ...
                    T* residuals_ptr) const
    {
        ...
        residuals.template block<3, 1>(3, 0) = T(2.0*weight) * delta_q.vec();

        return true;
    }
    static ceres::CostFunction* Create(const CeresPose& t_ab_measured, const float weight)
    {
        return new ceres::AutoDiffCostFunction<Averaging3dErrorTerm, 6, 3, 4, 3, 4>(new Averaging3dErrorTerm(t_ab_measured, weight));
    }
